...
async function saveSubscription(env: Env, input: SubscriptionInput) {
  ...
  const params = [name, url || null, content, input.target || 'raw', input.userAgent || null, JSON.stringify(input.config || {}), input.enabled === false ? 0 : 1, timestamp];
  if (existing) {
    await env.DB.prepare(`UPDATE subscriptions SET url=?, content=?, target=?, user_agent=?, config_json=?, enabled=?, updated_at=? WHERE name=?`)
      .bind(url || null, content, input.target || 'raw', input.userAgent || null, JSON.stringify(input.config || {}), input.enabled === false ? 0 : 1, timestamp, name).run();
  } else {
    await env.DB.prepare(`INSERT INTO subscriptions(name,url,content,target,user_agent,config_json,enabled,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)`)
      .bind(name, url || null, content, input.target || 'raw', input.userAgent || null, JSON.stringify(input.config || {}), input.enabled === false ? 0 : 1, timestamp, timestamp).run();
  }
...
app.get('/api/storage', admin, async (c) => {
  ...
  return json({ settings: {}, subscriptions: rows.results || [] }, { headers: { 'content-disposition': `attachment; filename="sub-store_data_${new Date().toISOString().slice(0,10)}.json"` } });
});
...
app.post('/api/subscriptions', admin, async (c) => {
  ...
  const row = await saveSubscription(c.env, input);
  return c.json({ ok: true, subscription: row }, 201);
});
app.get('/api/subscriptions/:name', admin, async (c) => {
  const name = validName(c.req.param('name'));
...
app.put('/api/subscriptions/:name', admin, async (c) => {
  const name = validName(c.req.param('name'));
  const input = await c.req.json<SubscriptionInput>();
  const row = await saveSubscription(c.env, { ...input, name });
...
app.all('/download/:name', async (c) => {
  ...
  if (!row) throw new HTTPException(404, { message: 'Subscription not found' });
  const queryTarget = c.req.query('target') || row.target || 'raw';
  const source = await fetchSource(c.env, row);
  const rendered = renderTarget(queryTarget, source);
  const headers = new Headers({ 'content-type': rendered.contentType, 'cache-control': `public, max-age=${Number(c.env.CACHE_TTL_SECONDS || 900)}` });
  headers.set('x-sub-store-cache', row.url ? 'remote' : 'inline');
  if (queryTarget === 'raw') headers.set('content-disposition', `inline; filename="${name}.txt"`);
...
app.get('/api/file/:name', async (c) => {
  ...
  if (!row) throw new HTTPException(404, { message: 'Subscription not found' });
  const source = await fetchSource(c.env, row);
  const rendered = renderTarget(c.req.query('target') || row.target, source);
  return new Response(rendered.body, { headers: { 'content-type': rendered.contentType } });
});
...
export default {
  ...
  async scheduled(_event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ctx.waitUntil((async () => {
      const staleBefore = now() - 7 * 24 * 60 * 60 * 1000;
      await env.DB.prepare('DELETE FROM subscriptions WHERE enabled = 0 AND updated_at < ?').bind(staleBefore).run();
      // Warm only lightweight metadata; actual subscription refresh occurs on download.
      await env.CACHE.put('health:last-cron', new Date().toISOString(), { expirationTtl: 3600 });
    })());
  }
};
